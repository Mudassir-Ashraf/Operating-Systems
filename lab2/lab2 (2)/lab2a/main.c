...
int main(int argc, char* argv[]) {
    ...
    if(argc == 1){
        exit_code = 1; // Set exit code to indicate too few arguments
    }
    else if(argc == 2){ // Set exit code to indicate a valid number of argument
        exit_code = 0;
    }
    else if(argc > 2){ // Set exit code to indicate too many arguments
        exit_code = 2;
    }
    ...
    if (argc > 1){
        person = argv[1]; //This takes the second argument value which would be Hello (name).
    }
    else {
        person = "UNKNOWN";
    }
    ...
    //if the last three characters are bye of the first argument
